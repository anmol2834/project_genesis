...
const FEATURES: Feature[] = [
  ...
  { label: 'Email processing credits/month', starter: '50 credits',  pro: '500 credits', enterprise: 'Unlimited', highlight: true },
  { label: 'Email accounts',                 starter: '1 account',   pro: '3 accounts',  enterprise: 'Unlimited' },
  { label: 'Leads / contacts',               starter: '100 leads',   pro: '2,000 leads', enterprise: 'Unlimited' },
  { label: 'Active campaigns',               starter: '1 campaign',  pro: '10 campaigns',enterprise: 'Unlimited' },
  // AI features
  { label: 'AI reply generation',            starter: true,          pro: true,          enterprise: true },
  { label: 'Zero-UI auto reply mode',         starter: true,          pro: true,          enterprise: true },
  // Inbox & Automation
  { label: 'Unified inbox',                  starter: true,          pro: true,          enterprise: true },
  { label: 'Campaign + inbox merge',          starter: false,         pro: true,          enterprise: true },
  // Analytics & Research
  { label: 'Basic analytics dashboard',       starter: true,          pro: true,          enterprise: true },
  { label: 'AI research assistant',           starter: false,         pro: true,          enterprise: true },
  // Data & Integrations
  { label: 'CSV lead import',                 starter: true,          pro: true,          enterprise: true },
  { label: 'CRM & third-party integrations',  starter: false,         pro: true,          enterprise: true },
  { label: 'My Data vault',                   starter: false,         pro: true,          enterprise: true },
  // Team & Security
  { label: 'Team members',                    starter: '1 member',    pro: '5 members',   enterprise: 'Unlimited' },
  { label: 'AES-256 token encryption',        starter: true,          pro: true,          enterprise: true },
  // Support
...
const PLANS = [
  ...
  },
  {
    id: 'enterprise',
    name: 'Enterprise',
    tagline: 'Unlimited scale, dedicated infra.',
    price: null,
    priceLabel: 'Custom',
    ...
    color: '#c084fc',
    ctaLabel: 'Contact us',
    ctaHref: 'mailto:sales@proxipilot.ai',
    ctaVariant: 'outlined' as const,
    topFeatures: [
      { icon: AllInclusiveRoundedIcon, text: 'Unlimited everything' },
  ...
  return (
    ...
    >
      <Box sx={{ maxWidth: 1100, mx: 'auto' }}>
        ...
        {/* ── Plan cards ── */}
        <StaggerContainer>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: 'repeat(3, 1fr)' }, gap: { xs: 1.5, sm: 2 }, alignItems: 'stretch', mb: { xs: 5, sm: 7 } }}>
            {PLANS.map((plan) => (
              <motion.div key={plan.id} variants={fadeUp} style={{ display: 'flex' }}>
                ...
                >
                  {/* Top gradient bar for highlighted plan */}
                  {plan.highlight && (
                  ...
                  {/* Badge */}
                  {plan.badge && (
                    <Box sx={{ position: 'absolute', top: 14, right: 14, px: 1.25, py: 0.4, borderRadius: '6px', background: grad.primary }}>
                      <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color: '#fff', letterSpacing: '0.04em' }}>{plan.badge}</Typography>
                    </Box>
                  )}

                  <Typography sx={{ fontWeight: 700, fontSize: '1.1rem', color: plan.color, mb: 0.5 }}>{plan.name}</Typography>
                  ...
                  <Box sx={{ mb: 2.5, px: 1.5, py: 1, borderRadius: '10px', background: alpha(plan.color, isDark ? 0.10 : 0.06), border: `1px dashed ${alpha(plan.color, isDark ? 0.32 : 0.22)}`, display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: plan.color }}>
                      {plan.id === 'starter' && '50 email credits included'}
                      {plan.id === 'pro' && '500 email credits / month'}
                      {plan.id === 'enterprise' && 'Unlimited email credits'}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, flex: 1 }}>
                    ...
                    {/* Extra plan-specific feature bullets */}
                    {plan.id === 'starter' && ([
                      '1 email account', 'Basic analytics dashboard', 'CSV lead import',
                    ]).map((f) => (
                    ...
                    {plan.id === 'pro' && ([
                      '3 email accounts · 2,000 leads', 'Context memory + tone adaptation', 'Campaigns, CRM integrations', 'Team (up to 5 members)', 'Priority email support',
                    ]).map((f) => (
                      <Box key={f} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                        <Box sx={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, mt: 0.1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: alpha(plan.color, isDark ? 0.16 : 0.09) }}>
                          <CheckRoundedIcon sx={{ fontSize: 10, color: plan.color }} />
                    ...
                    {plan.id === 'enterprise' && ([
                      'Unlimited accounts, leads, team', 'Custom AI training + RBAC', 'Compliance logs + audit trail', 'White-label + dedicated CSM',
                    ]).map((f) => (
                      <Box key={f} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                        <Box sx={{ width: 18, height: 18, borderRadius: '50%', flexShrink: 0, mt: 0.1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: alpha(plan.color, isDark ? 0.16 : 0.09) }}>
                          <CheckRoundedIcon sx={{ fontSize: 10, color: plan.color }} />
        ...
        <FadeUp delay={0.1}>
          <Box>
            ...
            <Box sx={{ borderRadius: '16px', border: `1px solid ${theme.palette.divider}`, overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
              <Box sx={{ minWidth: 520 }}>
                {/* Table header */}
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 110px 130px 110px', background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.025)', borderBottom: `1px solid ${theme.palette.divider}`, px: { xs: 1.5, sm: 2.5 }, py: 1.5 }}>
                ...
                {FEATURES.map((f, i) => (
                  <Box
                    key={f.label}
                    sx={{
                      display: 'grid', gridTemplateColumns: '1fr 110px 130px 110px',
                  ...
                  >
                    ...
                    <FeatureCell value={f.enterprise} color={PLANS[2].color} isDark={isDark} />
                  </Box>
                ))}
              </Box>
            </Box>
          ...
          <Box sx={{ mt: { xs: 4, sm: 5 }, p: { xs: 2.5, sm: 3 }, borderRadius: '16px', background: isDark ? `linear-gradient(135deg, ${alpha('#c084fc', 0.10)} 0%, ${alpha('#818cf8', 0.06)} 100%)` : `linear-gradient(135deg, ${alpha('#7c3aed', 0.05)} 0%, ${alpha('#4338ca', 0.03)} 100%)`, border: `1px solid ${alpha('#c084fc', isDark ? 0.25 : 0.18)}`, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { sm: 'center' }, gap: 2, justifyContent: 'space-between' }}>
            <Box>
              ...
              <Typography sx={{ fontSize: '0.82rem', color: 'text.secondary', maxWidth: 480 }}>
                Unlimited credits, dedicated infrastructure, custom AI training, SLA guarantees, white-label, and a dedicated customer success manager.
              </Typography>
            </Box>
            <Button
              component="a"
              href="mailto:sales@proxipilot.ai"
              variant="outlined"
