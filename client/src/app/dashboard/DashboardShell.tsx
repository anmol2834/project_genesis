    ...
    <ProtectedRoute>
      <Box sx={{ display: 'flex', height: '100svh', background: isDark ? '#080d18' : theme.palette.background.default, overflow: 'hidden' }}>
        ...
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', justifyContent: 'flex-end', gap: 1, px: 3, py: 1.5, flexShrink: 0, borderBottom: `1px solid ${theme.palette.divider}`, background: isDark ? 'rgba(8,13,24,0.9)' : theme.palette.background.paper }}>
            <IconButton onClick={() => router.push('/dashboard/notifications')} size="small" sx={{ width: 34, height: 34, color: 'text.secondary', borderRadius: '8px', border: `1px solid ${theme.palette.divider}`, position: 'relative', '&:hover': { borderColor: theme.palette.primary.main } }}>
