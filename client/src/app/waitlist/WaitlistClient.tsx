  ...
  return (
    ...
    >
      ...
      <Box sx={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        <Box sx={{ position: 'absolute', top: '10%', left: '5%', width: { xs: 200, md: 350 }, height: { xs: 200, md: 350 }, background: isDark ? 'radial-gradient(circle, rgba(139,92,246,0.15) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(139,92,246,0.08) 0%, transparent 70%)', animation: 'float 8s ease-in-out infinite' }} />
        <Box sx={{ position: 'absolute', bottom: '10%', right: '8%', width: { xs: 180, md: 300 }, height: { xs: 180, md: 300 }, background: isDark ? 'radial-gradient(circle, rgba(236,72,153,0.12) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(236,72,153,0.06) 0%, transparent 70%)', animation: 'float 10s ease-in-out infinite reverse' }} />
        <Box sx={{ position: 'absolute', top: '45%', right: '15%', width: { xs: 120, md: 200 }, height: { xs: 120, md: 200 }, background: isDark ? 'radial-gradient(circle, rgba(245,158,11,0.10) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(245,158,11,0.05) 0%, transparent 70%)', animation: 'float 12s ease-in-out infinite' }} />
      </Box>

      {/* Back Button */}
      <Box sx={{ position: 'sticky', top: 0, left: 0, zIndex: 10, pt: { xs: 1.5, md: 2.5 }, pl: { xs: 1, md: 2.5 }, pb: 0 }}>
      ...
      >
        ...
        >
          ...
          <MotionBox initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            ...
            <Typography variant="h1" sx={{ fontSize: { xs: '1.6rem', sm: '2.25rem', md: '2.75rem' }, fontWeight: 800, mb: 1.5, background: isDark ? 'linear-gradient(135deg, #f8fafc 0%, #c7d2fe 100%)' : 'linear-gradient(135deg, #0f172a 0%, #3730a3 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1.15, textAlign: { xs: 'center', md: 'left' } }}>
              Join the Future of Email Automation
            </Typography>

            <Typography sx={{ fontSize: { xs: '0.875rem', sm: '1rem' }, color: 'text.secondary', mb: { xs: 2, md: 3 }, lineHeight: 1.6 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1, md: 1.5 }, mb: { xs: 2, md: 3 } }}>
            ...
            {/* Stats — desktop only (mobile version renders below the form) */}
            <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: { xs: 2.5, md: 3 } }}>
              {stats.map((stat, i) => (
                ...
                <>
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, mb: 2.5 }}>
                      <Box sx={{ width: 44, height: 44, borderRadius: '12px', background: grad.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  ...
                  {/* Stats — mobile only, centered below the form */}
                  <Box
                    sx={{
                  ...
                  >
                    {stats.map((stat, i) => (
                      <MotionBox key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6 + i * 0.1 }} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.3 }}>
                          <stat.icon sx={{ fontSize: 14, color: 'primary.main' }} />
