...
logger = setup_logging("campaign-service")
config = get_config()
...
app = FastAPI(title="Campaign Service", version="1.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "campaign-service", "version": "1.0.0", "status": "running"}


if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=8006, reload=config.DEBUG)
