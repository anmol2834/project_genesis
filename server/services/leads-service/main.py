...
logger = setup_logging("leads-service")
config = get_config()


@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("Leads Service starting up...")
    await init_database()
    await init_redis()
    logger.info("Leads Service started successfully")
    yield
    logger.info("Leads Service shutting down...")
    await close_database()
    await close_redis()


app = FastAPI(title="Leads Service", version="1.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "leads-service", "version": "1.0.0", "status": "running"}


if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=8007, reload=config.DEBUG)
