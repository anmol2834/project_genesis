"""
Inbox Service - Inbox Management & Conversations
Port: 8005
"""
...
logger = setup_logging("inbox-service")
config = get_config()


@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("Inbox Service starting up...")
    await init_database()
    await init_redis()
    logger.info("Inbox Service started successfully")
    yield
...
app = FastAPI(title="Inbox Service", version="1.0.0", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "inbox-service", "version": "1.0.0", "status": "running"}


if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=8005, reload=config.DEBUG)
